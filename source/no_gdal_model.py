...
def tune_parameters(train_set_X, train_set_y, valid_set_X, valid_set_y, param_dict, logfile_path, iterations=5):
    ...
    for i in range(iterations):
        hist = run(train_set_X, train_set_y, depth=param_dict["depth"], kernel_size=param_dict["kernel_size"],
...
def try_params():
    POINTER_FILE_PATH = r"/home/arildsd/pointers/06"
...
def create_pointer_files(data_path, output_folder, train_size=0.6, valid_size=0.2, test_size=0.2, shuffle=True,
    ...
    Make txt files that point to images. Splits into training, validation and test sets.
    :param output_folder:
    :param random_seed:
    :param data_path:
    :param train_size:
